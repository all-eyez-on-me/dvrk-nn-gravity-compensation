...
for i = 1:size(input_mat,2)-fixWindowLength+1
    train_input_cell = vertcat(train_input_cell, {input_mat(:,i:i+fixWindowLength-1)});
    train_output_cell = vertcat(train_output_cell, {output_mat(:,i:i+fixWindowLength-1)});
end

train_input_mat = [train_input_mat input_mat(1:6,:)];
train_output_mat = [train_output_mat output_mat(1:6,:)];
