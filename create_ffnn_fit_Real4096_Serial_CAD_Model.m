...
net = configure(net, train_input_mat, train_output_mat_residual);


if(is_useParallelCPU)
    [net,tr] = train(net,train_input_mat, train_output_mat_residual,'useParallel','yes');
elseif(is_useParallelGPU)
    [net,tr] = train(net,train_input_mat, train_output_mat_residual,'useGPU','yes');
else
    [net,tr] = train(net,train_input_mat, train_output_mat_residual);
end


% test
y_residual = net(test_input_mat);
y_source_output = source_net(test_input_mat);
[abs_RMS_vec, rel_RMS_vec] = RMS(test_output_mat, y_source_output + y_residual)
